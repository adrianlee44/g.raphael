...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    if (opts.axis) {
        ...
        +ax[2] && axis.push(this.g.axis(x + gutter, y + height - gutter, width - 2 * gutter, minx, maxx, opts.axisxstep || Math.floor((width - 2 * gutter) / 20), 0, lbls[2], tags[2]));
        +ax[3] && axis.push(this.g.axis(x + gutter, y + height - gutter, height - 2 * gutter, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 1, lbls[3], tags[3]));
    }
    var lines = this.set(),
        symbols = this.set(),
        vertLine = this.set(),
